setwd('C:/Users/M/Desktop/Manuel/JOB/SPORTLOGiQ')
Match3Data <- read.csv("Mtl-Ott-game3.csv")
attach(Match3Data)

# libraries for plotly plotting
library("devtools")
library("plotly")
set_credentials_file("JMD", "ettwpv2emi")
...
# transform the Coordinates so everything hapens in the same half of the field
xCoordAdj = xCoord
yCoordAdj = yCoord
...
old_expG_Habs = AvGoalPerShot2014 * sum_shots_HabsONNET
old_expG_Senators = AvGoalPerShot2014 * sum_shots_SenatorsONNET

new_expG_Habs = sum_expG_Habs 
new_expG_Senators = sum_expG_Senators
...
# I modified the heatmap a bit (using GIMP) to get rid of the artifacts, and blurred and pixelated it a bit to get rid 
# of the white spaces. I also resized it to fit nicely in our 168 x 200 format, so that each pixel represents
# one of the possible x-y-coordinates

#hm=readPNG(paste0('C:/Users/M/Desktop/Manuel/JOB/SPORTLOGiQ/great_mini_hm.png'), FALSE)
hm=readPNG(paste0('C:/Users/M/Desktop/Manuel/JOB/SPORTLOGiQ/superMiniHeatmap.png'), FALSE)
# Limits of image
xbounds=c(1, ncol(hm))
ybounds=c(1, nrow(hm))
...
# And now lets rearange this date back to our 168x200 format
...
png("84.png")
col84 <- rgb(hm[84,,])
plot(Prob_Field[84,], col=col84, pch=20, cex=3)
dev.off()

png("1.png")
col1 <- rgb(hm[1,,])
plot(Prob_Field[1,], col=col1, pch=20, cex=3)
dev.off()

png("168.png")
col168 <- rgb(hm[168,,])
plot(Prob_Field[168,], col=col168, pch=20, cex=3)
dev.off()

png("42.png")
col42 <- rgb(hm[42,,])
plot(Prob_Field[42,], col=col42, pch=20, cex=3)
dev.off()

png("126.png")
col126 <- rgb(hm[126,,])
plot(Prob_Field[126,], col=col126, pch=20, cex=3)
dev.off()

hist(Prob_Field)

# This looks great! However, there are some outliers with values below 0 and with 26.53
...
Prob_Field[which(Prob_Field > 0.26)] = 0.01
...
while(i < numberOfShots){
  ...
  av_prob = av_prob + Prob_Field[this_y, this_x]
}
...
while(i < sum_shots_Habs){
  ...
  av_prob_Habs = av_prob_Habs + Prob_Field[this_y, this_x]
}
...
while(i < sum_shots_Senators){
  ...
  av_prob_Senators = av_prob_Senators + Prob_Field[this_y, this_x]
}
...
while(i < numberOfShotsONNET){
  ...
  av_probONNET = av_probONNET + Prob_Field[this_y, this_x]
}
...
while(i < sum_shots_HabsONNET){
  ...
  av_prob_HabsONNET = av_prob_HabsONNET + Prob_Field[this_y, this_x]
}
...
while(i < sum_shots_SenatorsONNET){
  ...
  av_prob_SenatorsONNET = av_prob_SenatorsONNET + Prob_Field[this_y, this_x]
}
...
new_expG_Habs
new_expG_Senators
# They all predict, that the Habs would have won given the amount of shots (and their given probabilities)
# However, our new stats predict less goals in the match and perform in this case better than the others
# as the actual score was 2:1
...
response <- py$plotly(data, kwargs=list(layout=layout, filename="line-style"))
url <- response$url
...
layout <- list(
  title = "Shot Location Match 3 Habs", 
  xaxis = list(
    title = "X Coordinate", 
    range = c(0, 200),
    showgrid = FALSE, 
...
response <- py$plotly(data, kwargs=list(layout=layout, filename="Shot location Habs ONNET"))
url <- response$url
...
layout <- list(
  title = "Shot Location Match 3 Senators", 
  xaxis = list(
    title = "X Coordinate", 
    range = c(0, 200),
    showgrid = FALSE, 
...
response <- py$plotly(data, kwargs=list(layout=layout, filename="ShotLocationSenatorsONNET"))
url <- response$url

####
colShotsHabsRGBONNET = rgb(colShotsHabsONNET[,])
png("Shots_Habs3ONNET3.png")
plot(xCoordNormShotHabsONNET, yCoordNormShotHabsONNET, col=colShotsHabsRGBONNET, pch=19, cex=3, xlim = c(0,200), ylim=c(0,168))
dev.off()

colShotsSenatorsRGBONNET = rgb(colShotsSenatorsONNET[,])
png("Shots_Senators3ONNET3.png")
